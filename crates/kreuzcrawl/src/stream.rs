...
///
/// Each page crawled emits a [`CrawlEvent::Page`] event. When the crawl is
/// complete, a [`CrawlEvent::Complete`] event is emitted.
pub fn crawl_stream(url: &str, config: &CrawlConfig) -> ReceiverStream<CrawlEvent> {
    ...
    tokio::spawn(async move {
        match crawl::crawl(&url, &config).await {
            Ok(result) => {
                let pages_crawled = result.pages.len();
                for page in result.pages {
                    if tx.send(CrawlEvent::Page(Box::new(page))).await.is_err() {
                        return;
                    }
                }
