...
// alef:hash:539701a7c61d2b74b2beab0ad4ae077eada37cd30feccf7796cc90d9f9feff30
...
impl WasmCrawlConfig {
    ...
    pub fn auth(&self) -> Option<String> {
        self.auth.as_ref().and_then(|v| {
            let api_config: kreuzcrawl::AuthConfig = v.clone().into();
            serde_json::to_string(&api_config).ok()
        })
    }
    ...
    pub fn asset_types(&self) -> Vec<String> {
        self.asset_types.iter().map(|v| v.to_api_str().to_string()).collect()
    }

    #[wasm_bindgen(setter, js_name = "assetTypes")]
    pub fn set_asset_types(&mut self, value: Vec<String>) {
        self.asset_types = value.into_iter().map(|s| WasmAssetCategory::from_api_str(&s)).collect();
    }
...
/// Authentication configuration (flexible struct for field-based construction).
#[wasm_bindgen]
#[derive(Clone, Default)]
...
impl WasmAuthConfig {
    ...
    pub fn set_type_field(&mut self, value: String) {
        self.r#type = value;
    }
...
impl From<kreuzcrawl::AuthConfig> for WasmAuthConfig {
    fn from(val: kreuzcrawl::AuthConfig) -> Self {
        match val {
            kreuzcrawl::AuthConfig::Basic { username, password } => WasmAuthConfig {
                r#type: "basic".to_string(),
                username: Some(username),
                password: Some(password),
                ..Default::default()
            },
            kreuzcrawl::AuthConfig::Bearer { token } => WasmAuthConfig {
                r#type: "bearer".to_string(),
                token: Some(token),
                ..Default::default()
            },
            kreuzcrawl::AuthConfig::Header { name, value } => WasmAuthConfig {
