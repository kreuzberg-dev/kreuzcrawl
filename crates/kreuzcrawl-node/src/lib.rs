...
// alef:hash:b855a880a58f2208c2d6ad12740e1b5fe4cbd053d33a33e1adad7b6074a7a6a0
...
pub async fn crawl_stream(engine: &JsCrawlEngineHandle, url: String) -> Result<CrawlStreamIterator> {
    let core_CrawlStreamRequest: kreuzcrawl::CrawlStreamRequest = JsCrawlStreamRequest {
        url: Some(url),
        ..Default::default()
    }
    .into();
    ...
    tokio::spawn(async move {
        use futures_util::StreamExt;
        match inner.crawl_stream(core_CrawlStreamRequest).await {
            Err(e) => {
                let _ = tx
                    .send(Err(napi::Error::new(napi::Status::GenericFailure, e.to_string())))
                    .await;
...
pub async fn batch_crawl_stream(engine: &JsCrawlEngineHandle, urls: Vec<String>) -> Result<BatchCrawlStreamIterator> {
    let core_BatchCrawlStreamRequest: kreuzcrawl::BatchCrawlStreamRequest = JsBatchCrawlStreamRequest {
        urls: Some(urls),
        ..Default::default()
    }
    .into();
    ...
    tokio::spawn(async move {
        use futures_util::StreamExt;
        match inner.batch_crawl_stream(core_BatchCrawlStreamRequest).await {
            Err(e) => {
                let _ = tx
                    .send(Err(napi::Error::new(napi::Status::GenericFailure, e.to_string())))
                    .await;
