...
// alef:hash:ee69c2ab9035785d7195df3f091bf46c67ca2bb41cc894f2f2011b0a8b6f3774
...
impl From<JsScrapeResult> for kreuzcrawl::ScrapeResult {
    fn from(val: JsScrapeResult) -> Self {
        ...
        __result.extracted_data = val.extracted_data.as_ref().and_then(|s| serde_json::from_str(s).ok());
        __result.extraction_meta = val.extraction_meta.map(Into::into);
        __result.screenshot = val.screenshot.map(Into::into);
...
impl From<JsMarkdownResult> for kreuzcrawl::MarkdownResult {
    fn from(val: JsMarkdownResult) -> Self {
        ...
        __result.document_structure = val
            .document_structure
            .as_ref()
            .and_then(|s| serde_json::from_str(s).ok());
        if let Some(__v) = val.tables {
            __result.tables = __v.into_iter().filter_map(|s| serde_json::from_str(&s).ok()).collect();
        }
