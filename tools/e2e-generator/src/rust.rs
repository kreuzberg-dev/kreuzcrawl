...
use crate::fixtures::{
    ArticleAssertions, Assertions, AssetAssertions, AuthAssertions, BatchAssertions, BrowserAssertions,
    ComputedAssertions, ContentAssertions, CookieAssertions, CrawlAssertions, DublinCoreAssertions, ErrorAssertions,
    ExtendedMetadataAssertions, ExtendedOgAssertions, FaviconAssertions, FeedAssertions, FilterAssertions, Fixture,
    HeadingAssertions, HreflangAssertions, ImageAssertions, JsonLdAssertions, LinkAssertions, MapAssertions,
    MarkdownAssertions, MetadataAssertions, OgAssertions, RateLimitAssertions, RedirectAssertions,
    ResponseMetaAssertions, RobotsAssertions, SitemapAssertions, StrategyAssertions, StreamAssertions,
    TwitterAssertions, ValidationAssertions,
};
...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path) -> Result<()> {
    ...
    std::fs::create_dir_all(&tests_dir).with_context(|| format!("creating directory {tests_dir}"))?;
    ...
    std::fs::write(rust_dir.join("Cargo.toml"), cargo_content).with_context(|| "writing e2e/rust/Cargo.toml")?;
    ...
    std::fs::write(src_dir.join("helpers.rs"), helpers_content).with_context(|| "writing e2e/rust/src/helpers.rs")?;

    let skipped = fixtures.iter().filter(|f| f.skip.is_some()).count();
    let generated = fixtures.len() - skipped;
    let categories = grouped.len();
    println!("Generated {generated} tests across {categories} categories in {rust_dir} ({skipped} skipped)");
...
fn generate_category_file(_category: &str, fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(out, "// Code generated by kreuzcrawl-e2e-generator. DO NOT EDIT.")?;
    ...
    let needs_feed_type = active_fixtures
        .iter()
        .any(|f| f.assertions.as_ref().and_then(|a| a.feeds.as_ref()).is_some());
    ...
    let needs_crawl_event = active_fixtures.iter().any(|f| {
        f.category() == "stream"
            || (f.category() == "engine" && f.assertions.as_ref().and_then(|a| a.stream.as_ref()).is_some())
    });
...
fn generate_test_fn(out: &mut String, fixture: &Fixture) -> Result<()> {
    ...
    if let Some(ref routes) = fixture.mock_responses {
        for (i, route) in routes.iter().enumerate() {
            ...
            let is_binary = route.body_file.as_ref().is_some_and(|f| is_binary_body_file(f));
            ...
            let headers_str = route
                .headers
                .iter()
                .map(|(k, v)| format!("(\"{}\", \"{}\")", escape_rust_string(k), escape_rust_string(v)))
                .collect::<Vec<_>>()
                .join(", ");
            if is_binary {
                writeln!(
    ...
    if category == "validation" {
        ...
        writeln!(out, "        .err().expect(\"build should fail with invalid config\");")?;
    } else if category == "rate_limit" {
        let delay_ms = fixture.config.as_ref().and_then(|c| c.rate_limit_delay_ms).unwrap_or(0);
        writeln!(
            out,
        ...
        if content_filter == "bm25" {
            let query = fixture
                ...
                .unwrap_or("");
            let threshold = fixture.config.as_ref().and_then(|c| c.bm25_threshold).unwrap_or(0.1);
            writeln!(
                out,
    ...
    match category {
        ...
        "stream" => {
            ...
            writeln!(out, "    let events: Vec<CrawlEvent> = stream.collect().await;")?;
        }
        "batch" => {
    ...
    if category == "validation" {
        ...
        writeln!(out, "    let result = result.expect(\"request should succeed\");")?;
        if let Some(ref assertions) = fixture.assertions {
            generate_assertions(out, assertions, category)?;
        }
...
fn generate_error_assertions(out: &mut String, fixture: &Fixture) -> Result<()> {
    ...
    if let Some(err) = error {
        if err.is_error == Some(true) {
            writeln!(out, "    let err = result.expect_err(\"request should fail\");")?;
...
fn generate_og_assertions(out: &mut String, og: &OgAssertions) -> Result<()> {
    ...
    if let Some(has) = og.has_og_description {
        if has {
            writeln!(out, "    assert!(result.metadata.og_description.is_some());")?;
        } else {
            writeln!(out, "    assert!(result.metadata.og_description.is_none());")?;
        }
    }
    Ok(())
}
...
fn generate_robots_assertions(out: &mut String, robots: &RobotsAssertions, _category: &str) -> Result<()> {
...
fn generate_sitemap_assertions(out: &mut String, sitemap: &SitemapAssertions) -> Result<()> {
    ...
    if let Some(has_lastmod) = sitemap.has_lastmod {
        if has_lastmod {
            writeln!(out, "    assert!(result.urls.iter().any(|u| u.lastmod.is_some()));")?;
        } else {
            writeln!(out, "    assert!(result.urls.iter().all(|u| u.lastmod.is_none()));")?;
        }
    }
    Ok(())
}
...
fn generate_crawl_assertions(out: &mut String, crawl: &CrawlAssertions) -> Result<()> {
    ...
    if let Some(stayed) = crawl.stayed_on_domain {
        if stayed {
            writeln!(out, "    assert!(result.pages.iter().all(|p| p.stayed_on_domain));")?;
        } else {
            writeln!(out, "    assert!(result.pages.iter().any(|p| !p.stayed_on_domain));")?;
        }
    }
    if let Some(count) = crawl.unique_normalized_urls {
        writeln!(out, "    assert_eq!(result.unique_normalized_urls(), {count});")?;
    }
    Ok(())
}
...
fn generate_redirect_assertions(out: &mut String, redirect: &RedirectAssertions) -> Result<()> {
    ...
    if let Some(is_err) = redirect.is_error {
        if is_err {
            writeln!(out, "    assert!(result.error.is_some(), \"expected redirect error\");")?;
        } else {
            writeln!(
                out,
...
fn generate_content_assertions(out: &mut String, content: &ContentAssertions) -> Result<()> {
    ...
    if let Some(main_only) = content.main_content_only {
        writeln!(out, "    assert_eq!(result.main_content_only, {main_only});")?;
    }
    Ok(())
}
...
fn generate_extended_metadata_assertions(out: &mut String, meta: &ExtendedMetadataAssertions) -> Result<()> {
...
fn generate_computed_assertions(out: &mut String, computed: &ComputedAssertions) -> Result<()> {
    if let Some(min) = computed.word_count_min {
        writeln!(out, "    assert!(result.metadata.word_count.unwrap_or(0) >= {min});")?;
    }
...
fn generate_response_meta_assertions(out: &mut String, meta: &ResponseMetaAssertions) -> Result<()> {
...
fn generate_rate_limit_assertions(out: &mut String, rate_limit: &RateLimitAssertions) -> Result<()> {
