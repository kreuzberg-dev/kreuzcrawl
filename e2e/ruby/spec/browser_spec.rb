...
# alef:hash:6041295288f60af0834e59542492b3ab3650e36d89e852dc4f2f0539656ebea1
...
RSpec.describe 'browser' do
  it 'browser_config_auto_no_feature: Browser mode \'auto\' without browser feature enabled does not use browser' do
    ...
    expect(result.js_render_hint.to_s).to be(true)
    expect(result.browser_used.to_s).to be(false)
  end
  ...
  it 'browser_crawl_mode_always: Crawl with browser mode \'always\' follows links using browser rendering' do
    ...
    expect(result.browser_used.to_s).to be(true)
  end
  ...
  it 'browser_detect_minimal_page: Does NOT flag a short but real content page as needing JS rendering' do
    ...
    result = Kreuzcrawl.scrape_async(engine, url)
    expect(result.status_code).to eq(200)
    expect(result.js_render_hint.to_s).to be(false)
    expect(result.browser_used.to_s).to be(false)
  end
