...
RSpec.describe "robots" do
  it "robots_allow_all: Permissive robots.txt allows all paths" do
    ...
    expect(result.robots.is_allowed).to eq(true)
  end
  ...
  it "robots_request_rate: Parses request-rate directive from robots.txt" do
    result = Kreuzcrawl.scrape()
    expect(result.robots.crawl_delay).to eq(5)
    expect(result.robots.is_allowed).to eq(true)
  end
  ...
  it "robots_x_robots_tag: Respects X-Robots-Tag HTTP header directives" do
    result = Kreuzcrawl.scrape()
    expect(result.robots.x_robots_tag).to eq("noindex, nofollow")
    expect(result.robots.noindex_detected).to eq(true)
    expect(result.robots.nofollow_detected).to eq(true)
  end
end
