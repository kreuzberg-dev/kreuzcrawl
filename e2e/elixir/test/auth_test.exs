...
defmodule E2e.AuthTest do
  ...
  describe "auth_basic_http" do
    test "Sends HTTP Basic authentication header" do
      engine_config = %Kreuzcrawl.CrawlConfig{auth: %{"password" => "testpass", "type" => "basic", "username" => "testuser"}, respect_robots_txt: false}
  ...
  describe "auth_bearer_token" do
    test "Sends Bearer token in Authorization header" do
      engine_config = %Kreuzcrawl.CrawlConfig{auth: %{"token" => "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test", "type" => "bearer"}, respect_robots_txt: false}
      {:ok, engine} = Kreuzcrawl.create_engine(engine_config)
  ...
  describe "auth_custom_header" do
    test "Sends authentication via custom header (X-API-Key)" do
      engine_config = %Kreuzcrawl.CrawlConfig{auth: %{"name" => "X-API-Key", "type" => "header", "value" => "sk-test-key-12345"}, respect_robots_txt: false}
      {:ok, engine} = Kreuzcrawl.create_engine(engine_config)
