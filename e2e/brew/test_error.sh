...
# alef:hash:126129f657b30ffa05da66f6ab6625234c18d0fac375ceb46bd41c760e19c1d6
...
test_error_invalid_proxy() {
  ...
  if kreuzcrawl scrape "${MOCK_SERVER_ERROR_INVALID_PROXY:-${MOCK_SERVER_URL}/fixtures/error_invalid_proxy}" --format json --browser-mode never >/dev/null 2>&1; then
    echo 'FAIL [error]: expected command to fail but it succeeded' >&2
    return 1
  fi
}
...
test_error_timeout() {
  ...
  if kreuzcrawl scrape "${MOCK_SERVER_ERROR_TIMEOUT:-${MOCK_SERVER_URL}/fixtures/error_timeout}" --format json --browser-mode never >/dev/null 2>&1; then
    echo 'FAIL [error]: expected command to fail but it succeeded' >&2
    return 1
  fi
}
