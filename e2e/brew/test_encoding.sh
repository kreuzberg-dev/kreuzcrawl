...
# alef:hash:2c900936fcc732b06c6fbe64379996e9b26a82f7b65106f7e8f75e0ca389f81d
...
test_encoding_double_encoded() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_ENCODING_DOUBLE_ENCODED:-${MOCK_SERVER_URL}/fixtures/encoding_double_encoded}" --format json --browser-mode never)
...
test_encoding_mixed_charset_page() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_ENCODING_MIXED_CHARSET_PAGE:-${MOCK_SERVER_URL}/fixtures/encoding_mixed_charset_page}" --format json --browser-mode never)
...
test_encoding_percent_encoded_path() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_ENCODING_PERCENT_ENCODED_PATH:-${MOCK_SERVER_URL}/fixtures/encoding_percent_encoded_path}" --format json --browser-mode never)
...
test_encoding_unicode_url() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_ENCODING_UNICODE_URL:-${MOCK_SERVER_URL}/fixtures/encoding_unicode_url}" --format json --browser-mode never)
