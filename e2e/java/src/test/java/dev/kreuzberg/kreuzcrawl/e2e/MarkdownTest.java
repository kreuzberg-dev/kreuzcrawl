...
// alef:hash:da7639f757823e676212278434ede1aaa95ccefe70293fa96ee7fa7b7def9c9a
...
class MarkdownTest {
    ...
    void testMarkdownBasicConversion() throws Exception {
        ...
        assertEquals("Test", java.util.Optional.ofNullable(result.metadata().title()).orElse("").trim());
        assertFalse(result.html().isEmpty(), "expected non-empty value");
        assertFalse(result.markdown().get().content().isEmpty(), "expected non-empty value");
        assertTrue(result.markdown().get().content().contains("Hello World"), "expected to contain: " + "Hello World");
    }
    ...
    void testMarkdownHeadingsAndParagraphs() throws Exception {
        ...
        var result = Kreuzcrawl.scrape(engine, url);
        assertFalse(result.markdown().get().content().isEmpty(), "expected non-empty value");
        assertTrue(result.markdown().get().content().contains("Main Title"), "expected to contain: " + "Main Title");
    }
    ...
    void testMarkdownLinksConverted() throws Exception {
        ...
        assertFalse(result.html().isEmpty(), "expected non-empty value");
        assertFalse(result.markdown().get().content().isEmpty(), "expected non-empty value");
        assertTrue(result.markdown().get().content().contains("Example"), "expected to contain: " + "Example");
    }
