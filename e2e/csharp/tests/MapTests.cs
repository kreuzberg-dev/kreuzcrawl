...
public class MapTests
{
    ...
    public async Task Test_MapExcludePatterns()
    {
        ...
        var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"exclude_paths\":[\"/private/.*\",\"/api/.*\"],\"max_depth\":0,\"respect_robots_txt\":false}")!;
        var engine = KreuzcrawlLib.CreateEngine(engineConfig);
    ...
    public async Task Test_MapIncludeSubdomains()
    {
        ...
        var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"allow_subdomains\":true,\"max_depth\":0,\"respect_robots_txt\":false}")!;
        var engine = KreuzcrawlLib.CreateEngine(engineConfig);
    ...
    public async Task Test_MapLimitPagination()
    {
        ...
        var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"map_limit\":5,\"max_depth\":0,\"respect_robots_txt\":false}")!;
        var engine = KreuzcrawlLib.CreateEngine(engineConfig);
    ...
    public async Task Test_MapSearchFilter()
    {
        ...
        var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"map_search\":\"blog\",\"max_depth\":0,\"respect_robots_txt\":false}")!;
        var engine = KreuzcrawlLib.CreateEngine(engineConfig);
