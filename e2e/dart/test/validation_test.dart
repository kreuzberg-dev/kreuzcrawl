...
void main() {
  ...
  test('Browser endpoint must be a valid ws:// or wss:// URL, not http://', () async {
    await expectLater(() async {
      ...
      final url = "${_sutUrl()}/fixtures/validation_browser_endpoint_invalid";
  ...
  test('auth object with empty username in basic auth is rejected', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_invalid_auth_config";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('Invalid regex in exclude_paths is rejected', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_invalid_exclude_regex";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('Invalid regex in include_paths is rejected', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_invalid_include_regex";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('proxy with invalid URL like \'not-a-url\' is rejected', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_invalid_proxy_url";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('Retry code outside 100-599 is rejected', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_invalid_retry_code";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('max_concurrent=0 is rejected as invalid config (minimum is 1)', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_max_concurrent_zero";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('max_depth=200 exceeds limit of 100', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_max_depth_too_high";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('max_pages=0 is rejected as invalid config', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_max_pages_zero";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('max_redirects > 100 is rejected as invalid config', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_max_redirects_too_high";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('max_body_size set to -1 is rejected as invalid config', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_negative_body_size";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
  ...
  test('Zero request timeout is rejected as invalid config', () async {
    await expectLater(() async {
      ...
      final engine = await KreuzcrawlBridge.createEngine(engineConfig);
      final url = "${_sutUrl()}/fixtures/validation_timeout_zero";
      return KreuzcrawlBridge.scrape(engine, url);
    }(), throwsA(anything));
  });
