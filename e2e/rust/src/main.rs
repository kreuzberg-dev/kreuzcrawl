...
// alef:hash:76ec4c955613adffc4025d63d15ca3b0a5212ddc9f5273d8706da0688aa920c8
            ...
            if let Some(arr) = input.get("mock_responses").and_then(|v| v.as_array()) {
                for entry in arr {
                    ...
                    let body: Option<serde_json::Value> = if let Some(inline) = entry.get("body_inline") {
                        ...
                        let abs = fixtures_dir.join(file);
                        match std::fs::read_to_string(&abs) {
                            Ok(s) => Some(serde_json::Value::String(s)),
                            Err(err) => {
                                eprintln!("warning: cannot read body_file {}: {err}", abs.display());
                                None
                            }
                        }
                    } else {
