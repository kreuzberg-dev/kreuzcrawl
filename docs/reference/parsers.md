...
Kreuzcrawl exposes substrate-level parsers for robots.txt and sitemap.xml that are usable without constructing the crawl engine. These are **Rust-only** surfaces and are not exposed in language bindings.

Import from `kreuzcrawl::robots` and `kreuzcrawl::sitemap` to use them with your own HTTP fetcher and pipeline.
...
```rust
use kreuzcrawl::sitemap::parse_sitemap_xml;
...
```rust
use kreuzcrawl::sitemap::{parse_sitemap_index, is_sitemap_index};
...
```rust
use kreuzcrawl::sitemap::is_sitemap_index;
