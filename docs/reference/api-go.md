...
| `RetryPolicy` | `*string` | `nil` | Pluggable per-attempt retry/escalation decision policy. Default is `new`. Not serializable — skip in TOML/JSON configs. |
| `WafClassifier` | `*string` | `nil` | Pluggable WAF classifier. Default is `builtin`. Not serializable — skip in TOML/JSON configs. |
| `DomainState` | `*string` | `nil` | Pluggable per-domain state backend. `nil` disables learning; the engine uses `SimpleRetryPolicy` semantics without state. Not serializable — skip in TOML/JSON configs. |
| `EscalationBudget` | `*string` | `nil` | Pluggable per-job escalation budget. `nil` means unlimited. Not serializable — skip in TOML/JSON configs. |
