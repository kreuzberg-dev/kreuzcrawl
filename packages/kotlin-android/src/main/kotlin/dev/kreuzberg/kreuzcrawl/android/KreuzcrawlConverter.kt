...
object KreuzcrawlConverter {
    ...
    fun generateCitations(markdown: String): CitationResult {
        val resultJson = KreuzcrawlConverterBridge.nativeGenerateCitations(markdown)
     ...
     */
    fun createEngine(config: CrawlConfig? = null): CrawlEngineHandle = CrawlEngineHandle(KreuzcrawlConverterBridge.nativeCreateEngine(config?.let { mapper.writeValueAsString(it) } ?: ""))
    /** Scrape a single URL, returning extracted page data. */
    fun scrape(engine: CrawlEngineHandle, url: String): ScrapeResult {
        val resultJson = KreuzcrawlConverterBridge.nativeScrape(engine.handle, url)
    ...
    fun crawl(engine: CrawlEngineHandle, url: String): CrawlResult {
        val resultJson = KreuzcrawlConverterBridge.nativeCrawl(engine.handle, url)
    ...
    fun mapUrls(engine: CrawlEngineHandle, url: String): MapResult {
        val resultJson = KreuzcrawlConverterBridge.nativeMapUrls(engine.handle, url)
    ...
    fun interact(engine: CrawlEngineHandle, url: String, actions: List<PageAction>): InteractionResult {
        val resultJson = KreuzcrawlConverterBridge.nativeInteract(engine.handle, url, mapper.writeValueAsString(actions))
    ...
    fun batchScrape(engine: CrawlEngineHandle, urls: List<String>): BatchScrapeResults {
        val resultJson = KreuzcrawlConverterBridge.nativeBatchScrape(engine.handle, mapper.writeValueAsString(urls))
    ...
    fun batchCrawl(engine: CrawlEngineHandle, urls: List<String>): BatchCrawlResults {
        val resultJson = KreuzcrawlConverterBridge.nativeBatchCrawl(engine.handle, mapper.writeValueAsString(urls))
