...
impl PageAction {
    ...
    pub fn _factory_wait(milliseconds: Option<i64>, selector: Option<String>) -> Self {
        Self::Wait { milliseconds, selector }
    }
...
fn ruby_init(ruby: &Ruby) -> Result<(), Error> {
    ...
    class.define_singleton_method("basic", function!(AuthConfig::_factory_basic, 2))?;

    class.define_singleton_method("bearer", function!(AuthConfig::_factory_bearer, 1))?;

    class.define_singleton_method("header", function!(AuthConfig::_factory_header, 2))?;

    let class = module.define_class("CrawlEvent", ruby.class_object())?;

    class.define_singleton_method("page", function!(CrawlEvent::_factory_page, 1))?;

    class.define_singleton_method("error", function!(CrawlEvent::_factory_error, 2))?;

    class.define_singleton_method("complete", function!(CrawlEvent::_factory_complete, 1))?;

    let class = module.define_class("PageAction", ruby.class_object())?;

    class.define_singleton_method("click", function!(PageAction::_factory_click, 1))?;

    class.define_singleton_method("type_text", function!(PageAction::_factory_type_text, 2))?;

    class.define_singleton_method("press", function!(PageAction::_factory_press, 1))?;

    class.define_singleton_method("scroll", function!(PageAction::_factory_scroll, 3))?;

    class.define_singleton_method("wait", function!(PageAction::_factory_wait, 2))?;

    class.define_singleton_method("screenshot", function!(PageAction::_factory_screenshot, 1))?;

    class.define_singleton_method("execute_js", function!(PageAction::_factory_execute_js, 1))?;
